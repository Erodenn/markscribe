...
function makeCreateNoteTool(container: ServiceContainer): ToolHandler {
  return {
    ...
    async handler(args): Promise<ToolResponse> {
      ...
      try {
        ...
        return {
          content: [{ type: "text", text: JSON.stringify({
            ...
            possibleSolutions: ["Check the required fields: path, content", "Ensure frontmatter values are valid types"],
          }) }],
          isError: true,
        };
      }
      ...
      try {
        await services.file.readNote(notePath);
        // If readNote succeeds, file already exists
        return {
          content: [{ type: "text", text: JSON.stringify({
            root: getRoot(container),
            error: `Note already exists: "${notePath}". Use write_note with overwrite mode for existing files.`,
            possibleSolutions: ["Use write_note with mode: 'overwrite' to update an existing note", "Choose a different path for the new note"],
          }) }],
          isError: true,
        };
      } catch (err) {
      ...
      if (explicitSchema !== undefined) {
        ...
        if (!found) {
          ...
          return {
            content: [{ type: "text", text: JSON.stringify({
              root: getRoot(container),
              error: `Note schema "${explicitSchema}" not found. Available note schemas: ${noteSchemas.map((s) => s.name).join(", ") || "(none)"}`,
              possibleSolutions: ["Use list_schemas to see available note schemas", "Omit noteSchema to auto-resolve from the convention cascade"],
            }) }],
            isError: true,
          };
        }
      ...
      try {
        await services.file.writeNote(notePath, content, finalFrontmatter, "overwrite");
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify(
              { root: getRoot(container), path: notePath, frontmatter: writtenFrontmatter, lintResult },
              null,
              2,
            ),
          },
