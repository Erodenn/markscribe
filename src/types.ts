...
/**
 * Mutable wrapper around Services so tool handler closures see directory switches.
 * Tools close over the container — on switch_directory, `services` is reassigned.
 */
...
export interface DirectoryStats {
  ...
  /** Total size in bytes */
  totalBytes: number;
  /** Most recently modified notes (relative paths) */
  recentFiles: Array<{ path: string; modified: string }>;
}

/**
 * All filesystem operations scoped to the root directory.
...
export interface FileService {
  ...
  /** Resolve relative path to absolute, with traversal check + PathFilter */
 ...
 * Constructor: (fileService: FileService, frontmatterService: FrontmatterService)
 */
...
export interface SchemaScope {
  /** Relative path prefixes this schema governs */
  paths: string[];
...
export interface FolderValidation {
  ...
  schema: string | null;
  /** Per-note lint results keyed by relative path */
...
export interface AreaValidation {
  /** Relative path of the scanned area */
  path: string;
  /** Whether all folders passed */
  pass: boolean;
  /** Per-folder results keyed by relative path */
  folders: Record<string, FolderValidation>;
  summary: {
    total: number;
    passed: number;
...
export interface SchemaEngine {
  ...
  /** Recursive validation of a subtree */
  validateArea(path: string): Promise<AreaValidation>;

  /** Validate entire directory tree using convention cascade */
  validateAll(): Promise<FullValidation>;
 ...
 * Key: relative path of the source note.
 * Value: array of relative paths the source links to.
 */
...
/** Shape of .markscribe/config.yaml */
export interface MarkScribeConfig {
  paths?: {
