...
async function makeTempVault(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "vaultscribe-schema-test-"));
}

function makeVaultService(vaultPath: string): VaultServiceImpl {
...
describe("SchemaEngineImpl.refresh()", () => {
  ...
  it("picks up a new schema file written after initial load", async () => {
    ...
    await writeFile(tmpDir, ".vaultscribe/schemas/test-note.yaml", MINIMAL_NOTE_SCHEMA_YAML);
  ...
  it("reflects updated schema content after refresh", async () => {
    ...
    await writeFile(tmpDir, ".vaultscribe/schemas/test-note.yaml", UPDATED_NOTE_SCHEMA_YAML);
...
content:
  rules: []
`;
    await writeFile(tmpDir, ".vaultscribe/schemas/daily-note.yaml", userOverride);
    await engine.loadSchemas(schemasDir);
    engine.loadBundledSchemas();
    await engine.discoverConventions();
...
noteSchemas:
  default: test-note
`;
    await writeFile(tmpDir, ".vaultscribe/schemas/project-folder.yaml", folderSchemaYaml);
    await writeFile(tmpDir, ".vaultscribe/schemas/test-note.yaml", MINIMAL_NOTE_SCHEMA_YAML);
    await writeFile(
      tmpDir,
  ...
  it("works correctly when called with no schemasDir (no user schemas)", async () => {
    // Don't call loadSchemas — simulate a vault with no .vaultscribe/schemas
    engine.loadBundledSchemas();
    await engine.discoverConventions();
