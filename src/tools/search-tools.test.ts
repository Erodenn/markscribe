...
async function makeTempVault(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "vaultscribe-search-tools-test-"));
}
...
describe("search_notes tool", () => {
  ...
  it("searches frontmatter when searchFrontmatter=true", async () => {
    ...
    const fmResults = JSON.parse(fmResult.content[0].text);
    const fmPaths = fmResults.map((r: { path: string }) => r.path);
    expect(fmPaths).toContain("note.md");

    // Default result should not find it if the word only appears in frontmatter
    const defaultResults = JSON.parse(defaultResult.content[0].text);
    const defaultPaths = defaultResults.map((r: { path: string }) => r.path);
