...
import type { Services, ToolHandler, VaultService } from "../types.js";
...
async function makeTempVault(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "vaultscribe-note-tools-"));
}

function makeVaultService(vaultPath: string): VaultService {
  const filter = new PathFilterImpl({ blockedPaths: [], allowedExtensions: [] });
  return new VaultServiceImpl(vaultPath, filter);
}

function makeServices(vault: VaultService): Services {
  const links = new LinkEngineImpl(vault);
  return { vault, links } as unknown as Services;
}
...
beforeEach(async () => {
  ...
  vault = makeVaultService(tmpDir);
  services = makeServices(vault);
...
describe("delete_note", () => {
  ...
  it("returns isError when confirmPath does not match", async () => {
    ...
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toMatch(/does not match/i);
